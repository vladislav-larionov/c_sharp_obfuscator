...
class CSharpObfuscator:
    ...
    METHOD_SIGNATURE_PATTERN = \
        "((public|private|protected|internal|unsafe|static|extern|async)\s)+\s*[a-zA-Z0-9_\[\]<>]+\s*[a-zA-Z0-9_]+\(([A-Za-z_0-9,\[\]<>\s]*)\)"
    PARAMETER_REPLACE_PATTERN = "(?<=[\. =+*/\-,;()\]\[]){0}(?=[ =+*/\-\.,;()\]\[])"
    CLASS_REPLACE_PATTERN = '{0}(?=(\\.|>|\\(| ))'
    VARIABLE_DEFINITION_PATTERN = "[a-zA-Z_0-9><\[\]]+\s[a-zA-Z0-9><_]+\s+=(?!=)"
    VARIABLE_IN_LOOP_DEFINITION_PATTERN = "[a-zA-Z_0-9><\[\]]+\s[a-zA-Z0-9><_]+\s+in(?=\s)"
    ...
    TO_REMOVE_SPACES_VARIABLE_DEFINITION_IN_LOOP_PATTERN = "([a-zA-Z_0-9><\[\]]+\s[a-zA-Z0-9><_]+\s+)in(?=\s)"
    ...
    def __rename_methods(self):
        ...
        for line in re.split('\n+', self.code):
            res = re.search(self.METHOD_SIGNATURE_PATTERN, line)
            if res:
                method_names.append(line[res.start():res.end()])
        method_name_dict = dict()
        for index, method_name in enumerate(method_names):
            old_name = method_name.split('(')[0].strip().split(' ')[-1].strip()
            if 'Main' != old_name:
                method_name_dict[old_name] = f"{self.NEW_METHOD_NAME_TEMPLATE}{self.method_count}"
            self.method_count += 1
    ...
    def __get_method_borders(self):
        ...
        method_borders = list()
        for match in re.finditer(self.METHOD_SIGNATURE_PATTERN, self.code):
